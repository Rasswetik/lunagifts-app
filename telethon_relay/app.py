...
# ---------- REVERSE PROXY (bypass Russian ISP blocks) ----------
import requests as _http

# Catch-all: proxy everything else to PythonAnywhere
@app.route('/', defaults={'path': ''})
@app.route('/<path:path>', methods=['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'PATCH'])
def _proxy(path):
    """Reverse proxy to PythonAnywhere for Russian ISP bypass."""
    target = f'{PA_URL}/{path}'
    qs = request.query_string.decode()
    if qs:
        target += '?' + qs

    # Forward headers, override Host
    fwd = {k: v for k, v in request.headers if k.lower() not in
           ('host', 'content-length', 'transfer-encoding')}
    fwd['Host'] = 'lunagifts.pythonanywhere.com'
    fwd['Accept-Encoding'] = 'identity'          # avoid gzip from origin
    ...
    try:
        ...
        return jsonify({'error': 'origin_unavailable', 'details': str(exc)}), 502

    ct = (resp.headers.get('Content-Type') or '').lower()
    excluded = {'content-encoding', 'transfer-encoding', 'content-length', 'connection'}

    # Rewrite URLs in text responses
    if any(t in ct for t in ('text/html', 'javascript', 'json')):
        body = resp.text
        proxy_origin = request.host_url.rstrip('/')
        body = body.replace('https://lunagifts.pythonanywhere.com', proxy_origin)
        body = body.replace('http://lunagifts.pythonanywhere.com', proxy_origin)
        hdrs = [(k, v) for k, v in resp.headers.items() if k.lower() not in excluded]
        return body, resp.status_code, hdrs

    # Redirect rewriting
    if 300 <= resp.status_code < 400:
        loc = resp.headers.get('Location', '')
        if 'lunagifts.pythonanywhere.com' in loc:
            loc = loc.replace('https://lunagifts.pythonanywhere.com', request.host_url.rstrip('/'))
            loc = loc.replace('http://lunagifts.pythonanywhere.com', request.host_url.rstrip('/'))
        hdrs = [(k, v) for k, v in resp.headers.items() if k.lower() not in excluded]
        hdrs = [(k, loc if k.lower() == 'location' else v) for k, v in hdrs]
        return '', resp.status_code, hdrs

    # Binary pass-through (images, CSS, etc.)
    hdrs = [(k, v) for k, v in resp.headers.items() if k.lower() not in excluded]
    return resp.content, resp.status_code, hdrs
